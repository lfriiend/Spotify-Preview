    ...
    <div>
      {tracks.items.map((item, index) => (
        ...
        >
          ...
          <span className="bg-zinc-50 w-20 h-11 rounded-2xl drop-shadow-2xl flex items-center justify-center text-textColor">{formatDuration(item.track.duration_ms)}</span>
        </div>
      ))}
    </div>
  )
